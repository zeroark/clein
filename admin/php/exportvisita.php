<?php
	...
	$querystring = 'SELECT visita.*, login.* FROM visita 
						join visita_login on visita_login.visita_id = visita.id
						join login on visita_login.login_id = login.id
						where visita.id = '.$id.' order by nombreyapellidoInput';
